...
module.exports = {
    ...
    edit: async (req, res) => {
        ...
        try {
            ...
            const editDogFood = await Food.findByIdAndUpdate(_id, req.body, {new:true} );
    ...
    total: async (req, res) => {
        ...
        try {
            const dogFood = await Food.find({
                "createdAt": {
                    $gte : new Date(year, month, date.getDate()),
                    $lte : new Date(year, month+1, date.getDate())
                }
            });
            ...
            const amount = [amountQuantity, amountPrice]
